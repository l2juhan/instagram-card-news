...
### cs-v2 / cs-doodle 카피 규칙 (요약)

cs-v2, cs-doodle(기본 스타일)로 만들 때는 `/card-news` 스킬의 "cs-v2 카피 규칙"을 따른다. 핵심:
...
> 기본 스타일이 `cs-doodle`이므로 `카드뉴스 만들어줘: {주제}`만 입력하면 cs-doodle로 생성됩니다. 다른 스타일은 요청에 이름을 적어 지정합니다 (예: "클린 스타일로", "cs-v2로").
