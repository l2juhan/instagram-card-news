...
/**
 * cs-doodle 엔진 코어. 팔레트/폰트/스프라이트/추가 타입만 테마별로 다르고 나머지
 * (chrome 마크업, 16종 공통 슬라이드 타입, seed 규칙, 변환 스크립트 주입)는 완전히
 * 같다는 전제로 scripts/build-cs-doodle.js의 로직을 테마 인자를 받는 함수로 옮긴 것.
 *
 * 안전장치: cs-doodle 테마는 원래 build-cs-doodle.js가 만들던 CSS 텍스트를 "그대로"
 * 넣어 두었다(변수 보간이 아니라 theme.cssRootExtra/cssRoleClasses/onDarkCss에 원문을
 * 통째로 박아 둠) — 그래서 cs-doodle 출력은 리팩터링 전후로 바이트 단위 동일해야 한다.
 * `node scripts/build-cs-doodle.js && git diff --stat templates/cs-doodle`로 검증한다.
 *
 * 사용법: 테마별 진입 스크립트(scripts/build-cs-doodle.js, scripts/build-aws.js)가
 *   require('./doodle/build-core.js').build(require('./doodle/themes/<name>.js'))
 * 를 호출한다.
 */
...
const { doodleScriptTag, objectSpriteMarkup } = require('./inline.js');
...
function build(theme) {
  const OUT_DIR = path.join(__dirname, '..', '..', 'templates', theme.name);

  // -------------------------------------------------------------------------
  // 정적 chrome: Node 시점에 한 번 구워서 고정한다 (seed 고정 → 항상 같은 모양).
  // 테마마다 별도 프로세스(node scripts/build-*.js)로 실행되므로 seedCounter는
  // 매번 5000부터 새로 시작한다 — cs-doodle 테마의 호출 순서를 원본과 그대로
  // 유지하는 한 나오는 seed 값도 원본과 같다.
  // -------------------------------------------------------------------------
  ...
  // -------------------------------------------------------------------------
  // 공용 CSS. 토큰 값과 role 클래스, on-dark 오버라이드는 theme이 원문 그대로 준다
  // (cs-doodle 테마는 build-cs-doodle.js 원본 문자열을 그대로 옮겨 바이트 동일성을 보장).
  // -------------------------------------------------------------------------
  const BASE_CSS = `
...
:root {
  ...
  --ink-soft: ${theme.tokens.inkSoft};
  --rule: ${theme.tokens.rule};
  --accent: {{accent_color}};
${theme.cssRootExtra}
...
`;
  ...
  // -------------------------------------------------------------------------
  // 슬라이드 타입 — 16종 공통 정의는 모든 테마가 같다. theme.extraTypes로 확장한다.
  // -------------------------------------------------------------------------
  const TYPES = {};
...
`,
  ...
  // extraTypes는 함수로 줄 수도 있다 — cs-doodle의 16종처럼 결정적 seed를 쓰는 chrome
  // SVG(hRuleSvg 등)를 테마 전용 타입(예: aws의 content-code)에서도 같은 seed 시퀀스로
  // 이어 쓰려면 build() 내부 클로저(R/nextSeed/chromeOpts/hRuleSvg/vRuleSvg/PROGRESS_W/NOTE)에
  // 접근해야 하기 때문이다. cs-doodle은 extraTypes: {}(빈 객체)라 이 경로를 타지 않는다.
  const extra = typeof theme.extraTypes === 'function'
    ? theme.extraTypes({ R, PEN, nextSeed, chromeOpts, hRuleSvg, vRuleSvg, PROGRESS_W, NOTE })
    : (theme.extraTypes || {});
  ...
  function buildHtml(type, def) {
    return `<!DOCTYPE html>
<!-- GENERATED by ${theme.generatedBy}: 이 파일을 직접 고치지 말고 빌더를 수정한 뒤 다시 생성할 것 -->
<html lang="ko">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=1080">
  <title>${theme.name} ${type}</title>
...
${objectSpriteMarkup(theme.spriteFiles)}
...
`;
  ...
  console.log(`${theme.name}: ${Object.keys(TYPES).length} templates written to ${OUT_DIR}`);
  return { TYPES };
}
