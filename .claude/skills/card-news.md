...
4. **구성 고정**: 1장 `cover` → 내용 → 끝에서 두 번째 `content-cheatsheet` → 마지막 `cta`. 권장 분량은 8~10장이다.
5. **CTA는 구체적 행동 하나**를 요청한다. 예: "시험 전에 같이 공부하는 친구한테 보내기". "팔로우", "좋아요" 같은 막연한 요청은 쓰지 않는다. `next_topic`에 다음 주제 예고를 한 줄 넣는다.
6. **치트시트**는 저장해 두고 다시 볼 한 장 요약이다. 행마다 라벨 1개, 값 한 줄로 쓰고 문장을 쓰지 않는다.
...
{검색 키워드}: {표지 훅을 풀어쓴 한 문장}

{본문 2~4줄: 이 캐러셀에서 얻는 것, 핵심 한 줄 요약}
{저장이나 공유를 부르는 한 줄 (CTA와 같은 행동)}
...
규칙:
- **첫 줄에 검색 키워드**를 넣는다. 예: "Diffie-Hellman 키 교환"
